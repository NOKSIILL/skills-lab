...
function resetReactionTest() {
  ...
  if (area && text) {
    ...
    text.textContent = "클릭해서 시작하세요!<br>Click to start!";
...
function handleReactionClick() {
  ...
  if (!reactionState.isWaiting) {
    ...
    text.textContent =
      "초록색이 되면 클릭하세요!<br>Click when it turns green!";
    reactionState.isWaiting = true;

    setTimeout(() => {
      if (reactionState.isWaiting) {
        area.className = "reaction-area ready";
        text.textContent = "지금 클릭!<br>Click now!";
    ...
    text.textContent = `${reactionTime}ms!  클릭해서 다시 시도<br>${reactionTime}ms! Click to try again`;
    area.className = "reaction-area";
    reactionState.isWaiting = false;
