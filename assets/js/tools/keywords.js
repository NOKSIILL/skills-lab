// 키워드 생성기
const keywordData = {
  ko: {
    nouns: [
      "바다",
      "산",
      "별",
      "꿈",
      "여행",
      "책",
      "음악",
      "햇살",
      "바람",
      "구름",
      "꽃",
      "나무",
      "새",
      "달",
      "강",
      "숲",
      "길",
      "집",
      "친구",
      "사랑",
      "시간",
      "기억",
      "빛",
      "어둠",
      "파도",
      "모래",
      "돌",
      "언덕",
      "초원",
      "호수",
      "섬",
      "폭포",
      "계곡",
      "들판",
      "정원",
      "창문",
      "문",
      "계단",
      "다리",
      "항구",
      "시장",
      "거리",
      "광장",
      "운동장",
      "카페",
      "식당",
      "부엌",
      "침대",
      "의자",
      "테이블",
      "책상",
      "연필",
      "펜",
      "종이",
      "그림",
      "사진",
      "영화",
      "극장",
      "무대",
      "관객",
      "연극",
      "소설",
      "시",
      "시인",
      "작가",
      "화가",
      "음악가",
      "멜로디",
      "리듬",
      "박자",
      "가사",
      "노래",
      "연주",
      "악기",
      "피아노",
      "기타",
      "드럼",
      "바이올린",
      "색",
      "색깔",
      "무늬",
      "패턴",
      "질감",
      "온도",
      "냄새",
      "향기",
      "맛",
      "소리",
      "속도",
      "힘",
      "무게",
      "크기",
      "모양",
      "원",
      "사각형",
      "삼각형",
      "선",
      "점",
      "그림자",
      "반사",
      "유리",
      "철",
      "나사",
      "전선",
      "전구",
      "시계",
      "열쇠",
      "상자",
      "가방",
      "지갑",
      "옷",
      "신발",
      "모자",
      "안경",
      "거울",
      "카메라",
      "핸드폰",
      "컴퓨터",
      "화면",
      "키보드",
      "마우스",
      "인터넷",
      "웹",
      "앱",
      "서버",
      "데이터",
      "파일",
      "서류",
      "계약",
      "증서",
      "지도",
      "나침반",
      "별자리",
      "태양",
      "은하",
      "우주",
      "행성",
      "위성",
      "로켓",
      "비행기",
      "자동차",
      "자전거",
      "오토바이",
      "배",
      "기차",
      "지하철",
      "버스",
      "택시",
      "도로",
      "교차로",
      "신호등",
      "주차장",
      "정류장",
      "탑",
      "성",
      "궁전",
      "사원",
      "절",
      "무덤",
      "박물관",
      "미술관",
      "도서관",
      "학교",
      "교실",
      "강의",
      "시험",
      "학원",
      "연구소",
      "실험",
      "프로젝트",
      "작업실",
      "공장",
      "창고",
      "사무실",
      "회의실",
      "팀",
      "조직",
      "회사",
      "브랜드",
      "상품",
      "제품",
      "포장",
      "라벨",
      "가격",
      "지불",
      "계좌",
      "카드",
      "은행",
      "돈",
      "동전",
      "지폐",
      "선물",
      "기프트",
      "축제",
      "파티",
      "생일",
      "결혼",
      "약속",
      "편지",
      "이메일",
      "메시지",
      "전화",
      "통화",
      "대화",
      "비밀",
      "거짓",
      "진실",
      "질문",
      "답변",
      "토론",
      "논리",
      "아이디어",
      "발명",
      "기술",
      "혁신",
      "도전",
      "목표",
      "계획",
      "성공",
      "실패",
      "변화",
      "시작",
      "끝",
      "출발",
      "도착",
      "여정",
      "항해",
      "탐험",
      "발견",
      "지도자",
      "영웅",
      "악당",
      "전쟁",
      "평화",
      "침묵",
      "소식",
      "뉴스",
      "언론",
      "정치",
      "사회",
      "경제",
      "문화",
      "역사",
      "전통",
      "풍습",
      "종교",
      "신화",
      "전설",
      "영감",
      "감정",
      "기쁨",
      "슬픔",
      "분노",
      "두려움",
      "호기심",
      "희망",
      "절망",
      "용기",
      "불안",
      "안정",
      "평온",
      "열정",
      "관심",
      "집중",
      "창조",
      "표현",
      "연결",
      "관계",
      "가족",
      "부모",
      "형제",
      "자매",
      "아이",
      "어른",
      "노인",
      "소년",
      "소녀",
      "청년",
      "직업",
      "의사",
      "간호사",
      "교사",
      "엔지니어",
      "프로그래머",
      "디자이너",
      "마케터",
      "요리사",
      "제빵사",
      "농부",
      "어부",
      "화물",
      "배달",
      "운동선수",
      "음식",
      "과일",
      "채소",
      "밥",
      "국",
      "찌개",
      "빵",
    ],

    adjectives: [
      "아름다운",
      "신비로운",
      "따뜻한",
      "차가운",
      "밝은",
      "어두운",
      "큰",
      "작은",
      "빠른",
      "느린",
      "새로운",
      "오래된",
      "깊은",
      "높은",
      "넓은",
      "좁은",
      "강한",
      "약한",
      "부드러운",
      "거친",
      "달콤한",
      "쓴",
      "짠",
      "시큼한",
      "매운",
      "상큼한",
      "진한",
      "연한",
      "조용한",
      "시끄러운",
      "부유한",
      "가난한",
      "건강한",
      "아픈",
      "행복한",
      "슬픈",
      "화난",
      "평온한",
      "불안한",
      "용감한",
      "겸손한",
      "자랑스러운",
      "수상한",
      "반짝이는",
      "희미한",
      "선명한",
      "투명한",
      "불투명한",
      "촉촉한",
      "건조한",
      "촉박한",
      "여유로운",
      "깜찍한",
      "귀여운",
      "멋진",
      "우아한",
      "투박한",
      "섬세한",
      "단순한",
      "복잡한",
      "현대적인",
      "고풍스러운",
      "전통적인",
      "미래적인",
      "낭만적인",
      "현실적인",
      "판타지 같은",
      "환상적인",
      "현실감 있는",
      "추상적인",
      "구체적인",
      "논리적인",
      "감성적인",
      "직관적인",
      "합리적인",
      "모험적인",
      "안정적인",
      "창의적인",
      "독창적인",
      "평범한",
      "특별한",
      "친근한",
      "낯선",
      "익숙한",
      "낭비하는",
      "절약하는",
      "풍부한",
      "절박한",
      "섬광같은",
      "조용조용한",
      "강렬한",
      "은은한",
      "달빛 같은",
      "햇살 같은",
      "바람 같은",
      "물결 같은",
      "꽃 같은",
      "숲 같은",
      "도시적인",
      "전원적인",
      "정교한",
      "거대한",
      "아담한",
      "미세한",
      "눈부신",
      "우울한",
      "명랑한",
      "차분한",
      "열정적인",
      "무심한",
      "성실한",
      "게으른",
      "호기심 많은",
      "무례한",
      "친절한",
      "냉정한",
      "따분한",
      "흥미로운",
      "도전적인",
      "수줍은",
      "자유로운",
      "구속된",
      "균형 잡힌",
      "불균형한",
      "안락한",
      "불편한",
      "편리한",
      "이상적인",
      "실용적인",
      "장식적인",
      "감각적인",
      "무거운",
      "가벼운",
      "단단한",
      "무른",
      "탄력 있는",
      "부서지기 쉬운",
      "확고한",
      "휘청거리는",
      "안정된",
      "유연한",
      "딱딱한",
      "반복적인",
      "일시적인",
      "영원한",
      "순간적인",
      "예측 가능한",
      "불확실한",
      "신속한",
      "지연된",
      "계획된",
      "즉흥적인",
      "공식적인",
      "비공식적인",
      "정확한",
      "대략적인",
      "철저한",
      "부정확한",
      "정직한",
      "교활한",
      "순수한",
      "타락한",
      "정중한",
      "감미로운",
      "거친맛의",
      "세련된",
      "촌스러운",
      "감각 없는",
      "우수한",
      "보통의",
      "미완성의",
      "완성된",
      "개성 있는",
      "무미건조한",
      "낙관적인",
      "비관적인",
      "관대한",
      "인색한",
      "열린",
      "닫힌",
      "활발한",
      "소극적인",
      "억제된",
      "자발적인",
      "협조적인",
      "독립적인",
      "유머러스한",
      "엄숙한",
      "호화로운",
      "소박한",
      "극적인",
      "성급한",
      "신중한",
      "번잡한",
      "한적한",
      "매혹적인",
      "불쾌한",
      "불가사의한",
      "무의미한",
      "의미 있는",
      "도도한",
      "겸허한",
      "전율적인",
      "평범하지 않은",
      "현저한",
      "미미한",
      "인위적인",
      "자연스러운",
      "현란한",
      "즉각적인",
      "지속적인",
      "일회성의",
      "체계적인",
      "무계획적인",
      "실험적인",
      "보수적인",
      "혁신적인",
      "보편적인",
      "특이한",
      "유명한",
      "무명의",
      "화려한",
      "수수한",
      "정적인",
      "동적인",
      "미래지향적인",
      "과거지향적인",
      "시적인",
      "산문적인",
      "희귀한",
      "흔한",
      "유기적인",
      "무생물적인",
      "유쾌한",
      "식상한",
      "신선한",
      "도취된",
      "뜨거운",
      "서늘한",
      "상징적인",
      "묘사적인",
      "해학적인",
      "유려한",
      "효율적인",
      "비효율적인",
      "균등한",
      "불균등한",
      "풍성한",
      "빈약한",
      "과감한",
      "소심한",
      "감동적인",
      "냉담한",
      "감정적인",
      "이성적인",
      "즉시의",
      "느긋한",
      "섬광의",
      "미소 짓는",
      "눈물 나는",
      "숨 막히는",
      "가슴 뛰는",
      "잔잔한",
      "요동치는",
      "반복되는",
      "드문",
      "일반적인",
      "특정한",
      "희망찬",
      "절망적인",
      "귀중한",
      "하찮은",
      "소중한",
      "무가치한",
      "치열한",
      "온화한",
      "격렬한",
      "완만한",
      "급격한",
      "연결된",
      "단절된",
      "탄생의",
      "소멸의",
      "재생의",
      "노화의",
      "순환하는",
      "단색의",
      "다채로운",
      "광택 있는",
      "무광의",
      "윤기 나는",
      "거칠거칠한",
      "매끈한",
      "굳건한",
      "빛나는",
      "어슴푸레한",
      "몽환적인",
      "유토피아적인",
    ],
  },
  en: {
    nouns: [
      "sea",
      "mountain",
      "star",
      "dream",
      "travel",
      "book",
      "music",
      "sunlight",
      "wind",
      "cloud",
      "flower",
      "tree",
      "bird",
      "moon",
      "river",
      "forest",
      "road",
      "house",
      "friend",
      "love",
      "time",
      "memory",
      "light",
      "darkness",
      "wave",
      "sand",
      "stone",
      "hill",
      "meadow",
      "lake",
      "island",
      "waterfall",
      "valley",
      "field",
      "garden",
      "window",
      "door",
      "stairs",
      "bridge",
      "port",
      "market",
      "street",
      "square",
      "playground",
      "cafe",
      "restaurant",
      "kitchen",
      "bed",
      "chair",
      "table",
      "desk",
      "pencil",
      "pen",
      "paper",
      "picture",
      "photo",
      "movie",
      "theater",
      "stage",
      "audience",
      "play",
      "novel",
      "poem",
      "poet",
      "writer",
      "painter",
      "musician",
      "melody",
      "rhythm",
      "beat",
      "lyrics",
      "song",
      "performance",
      "instrument",
      "piano",
      "guitar",
      "drum",
      "violin",
      "color",
      "color",
      "pattern",
      "pattern",
      "texture",
      "temperature",
      "smell",
      "fragrance",
      "taste",
      "sound",
      "speed",
      "strength",
      "weight",
      "size",
      "shape",
      "circle",
      "rectangle",
      "triangle",
      "line",
      "point",
      "shadow",
      "reflection",
      "glass",
      "iron",
      "screw",
      "wire",
      "light bulb",
      "clock",
      "key",
      "box",
      "bag",
      "wallet",
      "clothes",
      "shoes",
      "hat",
      "glasses",
      "mirror",
      "camera",
      "mobile phone",
      "computer",
      "screen",
      "keyboard",
      "mouse",
      "internet",
      "web",
      "app",
      "server",
      "data",
      "file",
      "document",
      "contract",
      "certificate",
      "map",
      "compass",
      "constellation",
      "sun",
      "galaxy",
      "universe",
      "planet",
      "satellite",
      "rocket",
      "airplane",
      "car",
      "bicycle",
      "motorcycle",
      "ship",
      "train",
      "subway",
      "bus",
      "taxi",
      "road",
      "intersection",
      "traffic light",
      "parking lot",
      "station",
      "tower",
      "castle",
      "palace",
      "temple",
      "temple",
      "tomb",
      "museum",
      "art gallery",
      "library",
      "school",
      "classroom",
      "lecture",
      "exam",
      "academy",
      "research institute",
      "experiment",
      "project",
      "workshop",
      "factory",
      "warehouse",
      "office",
      "meeting room",
      "team",
      "organization",
      "company",
      "brand",
      "product",
      "product",
      "packaging",
      "label",
      "price",
      "payment",
      "account",
      "card",
      "bank",
      "money",
      "coin",
      "bill",
      "gift",
      "gift",
      "festival",
      "party",
      "birthday",
      "marriage",
      "promise",
      "letter",
      "email",
      "message",
      "phone",
      "call",
      "conversation",
      "secret",
      "lie",
      "truth",
      "question",
      "answer",
      "discussion",
      "logic",
      "idea",
      "invention",
      "technology",
      "innovation",
      "challenge",
      "goal",
      "plan",
      "success",
      "failure",
      "change",
      "start",
      "end",
      "departure",
      "arrival",
      "journey",
      "voyage",
      "exploration",
      "discovery",
      "leader",
      "hero",
      "villain",
      "war",
      "peace",
      "silence",
      "news",
      "news",
      "media",
      "politics",
      "society",
      "economy",
      "culture",
      "history",
      "tradition",
      "custom",
      "religion",
      "myth",
      "legend",
      "inspiration",
      "emotion",
      "joy",
      "sadness",
      "anger",
      "fear",
      "curiosity",
      "hope",
      "despair",
      "courage",
      "anxiety",
      "stability",
      "tranquility",
      "passion",
      "interest",
      "concentration",
      "creation",
      "expression",
      "connection",
      "relationship",
      "family",
      "parent",
      "brother",
      "sister",
      "child",
      "adult",
      "elderly",
      "boy",
      "girl",
      "youth",
      "job",
      "doctor",
      "nurse",
      "teacher",
      "engineer",
      "programmer",
      "designer",
      "marketer",
      "chef",
      "baker",
      "farmer",
      "fisherman",
      "cargo",
      "delivery",
      "athlete",
      "food",
      "fruit",
      "vegetable",
      "rice",
      "soup",
      "stew",
      "bread",
    ],
    adjectives: [
      "beautiful",
      "mysterious",
      "warm",
      "cold",
      "bright",
      "dark",
      "big",
      "small",
      "fast",
      "slow",
      "new",
      "old",
      "deep",
      "high",
      "wide",
      "narrow",
      "strong",
      "weak",
      "soft",
      "rough",
      "sweet",
      "bitter",
      "salty",
      "sour",
      "spicy",
      "refreshing",
      "thick",
      "light",
      "quiet",
      "noisy",
      "rich",
      "poor",
      "healthy",
      "sick",
      "happy",
      "sad",
      "angry",
      "peaceful",
      "anxious",
      "brave",
      "humble",
      "proud",
      "suspicious",
      "shining",
      "faint",
      "clear",
      "transparent",
      "opaque",
      "moist",
      "dry",
      "urgent",
      "relaxed",
      "cute",
      "adorable",
      "cool",
      "elegant",
      "crude",
      "delicate",
      "simple",
      "complex",
      "modern",
      "antique",
      "traditional",
      "futuristic",
      "romantic",
      "realistic",
      "fantasy-like",
      "fantastic",
      "realistic",
      "abstract",
      "specific",
      "logical",
      "emotional",
      "intuitive",
      "rational",
      "adventurous",
      "stable",
      "creative",
      "original",
      "ordinary",
      "special",
      "friendly",
      "strange",
      "familiar",
      "wasteful",
      "economical",
      "abundant",
      "desperate",
      "lightning-like",
      "very quiet",
      "intense",
      "subtle",
      "moonlight-like",
      "sunlight-like",
      "wind-like",
      "wave-like",
      "flower-like",
      "forest-like",
      "urban",
      "rural",
      "precise",
      "huge",
      "cozy",
      "minute",
      "dazzling",
      "depressed",
      "cheerful",
      "calm",
      "passionate",
      "indifferent",
      "diligent",
      "lazy",
      "curious",
      "rude",
      "kind",
      "cold-hearted",
      "boring",
      "interesting",
      "challenging",
      "shy",
      "free",
      "confined",
      "balanced",
      "unbalanced",
      "comfortable",
      "uncomfortable",
      "convenient",
      "ideal",
      "practical",
      "decorative",
      "sensual",
      "heavy",
      "light",
      "hard",
      "soft",
      "elastic",
      "fragile",
      "firm",
      "swaying",
      "stable",
      "flexible",
      "stiff",
      "repetitive",
      "temporary",
      "eternal",
      "momentary",
      "predictable",
      "uncertain",
      "swift",
      "delayed",
      "planned",
      "improvised",
      "formal",
      "informal",
      "accurate",
      "approximate",
      "thorough",
      "inaccurate",
      "honest",
      "cunning",
      "pure",
      "corrupt",
      "polite",
      "sweet",
      "harsh-tasting",
      "sophisticated",
      "rustic",
      "tasteless",
      "excellent",
      "ordinary",
      "incomplete",
      "complete",
      "unique",
      "bland",
      "optimistic",
      "pessimistic",
      "generous",
      "stingy",
      "open",
      "closed",
      "active",
      "passive",
      "restrained",
      "spontaneous",
      "cooperative",
      "independent",
      "humorous",
      "solemn",
      "luxurious",
      "simple",
      "dramatic",
      "hasty",
      "cautious",
      "busy",
      "secluded",
      "charming",
      "unpleasant",
      "mysterious",
      "meaningless",
      "meaningful",
      "arrogant",
      "humble",
      "thrilling",
      "extraordinary",
      "remarkable",
      "minor",
      "artificial",
      "natural",
      "flashy",
      "modulated",
      "immediate",
      "continuous",
      "one-time",
      "systematic",
      "unplanned",
      "experimental",
      "conservative",
      "innovative",
      "universal",
      "peculiar",
      "famous",
      "unknown",
      "splendid",
      "plain",
      "static",
      "dynamic",
      "future-oriented",
      "past-oriented",
      "poetic",
      "prosaic",
      "rare",
      "common",
      "organic",
      "inanimate",
      "pleasant",
      "stale",
      "fresh",
      "intoxicated",
      "hot",
      "cool",
      "symbolic",
      "descriptive",
      "humorous",
      "fluent",
      "efficient",
      "inefficient",
      "equal",
      "unequal",
      "abundant",
      "poor",
      "bold",
      "timid",
      "touching",
      "indifferent",
      "emotional",
      "rational",
      "instant",
      "leisurely",
      "flashy",
      "smiling",
      "tearful",
      "breathtaking",
      "heart-pounding",
      "gentle",
      "fluctuating",
      "repeating",
      "rare",
      "general",
      "specific",
      "hopeful",
      "hopeless",
      "precious",
      "trivial",
      "valuable",
      "worthless",
      "fierce",
      "gentle",
      "violent",
      "gentle",
      "sudden",
      "connected",
      "disconnected",
      "birth",
      "extinction",
      "regeneration",
      "aging",
      "circulating",
      "monochrome",
      "colorful",
      "glossy",
      "matte",
      "shiny",
      "rough",
      "smooth",
      "firm",
      "shining",
      "dim",
      "dreamy",
      "utopian",
    ],
  },
};

// 키워드 생성
function generateKeywords() {
  const grid = document.getElementById("keywordGrid");
  if (!grid) return;

  // 현재 언어 감지
  const currentLang = document.documentElement.lang || "ko";
  const langData = keywordData[currentLang] || keywordData.ko; // 기본값은 한국어

  grid.innerHTML = "";

  // 선택된 언어의 키워드로 생성
  const selectedNouns = shuffleArray(langData.nouns).slice(0, 2);
  const selectedAdjectives = shuffleArray(langData.adjectives).slice(0, 2);

  [...selectedNouns, ...selectedAdjectives].forEach((keyword) => {
    const tag = document.createElement("div");
    tag.className = "keyword-tag";
    tag.innerHTML = keyword;

    tag.addEventListener("mouseenter", () => {
      tag.style.background = "rgba(0,255,136,0.3)";
      tag.style.transform = "translateY(-2px)";
    });

    tag.addEventListener("mouseleave", () => {
      tag.style.background = "rgba(0,255,136,0.2)";
      tag.style.transform = "translateY(0)";
    });

    grid.appendChild(tag);
  });
}

// 키워드 복사
function copyKeywords() {
  const tags = document.querySelectorAll(".keyword-tag");
  const keywords = Array.from(tags).map((tag) => tag.innerHTML);
  const currentLang = document.documentElement.lang || "ko";

  const message =
    currentLang === "en"
      ? "Keywords have been copied!"
      : "키워드가 복사되었습니다!";

  copyToClipboard(keywords.join(", "))
    .then(() => {
      showCopyNotification(message);
    })
    .catch(() => {
      const errorMsg =
        currentLang === "en"
          ? "Failed to copy. Please try again."
          : "복사에 실패했습니다. 다시 시도해주세요.";
      alert(errorMsg);
    });
}
